...
- A private secret directory containing every filename present under `docker/secrets/examples/`.

The committed example secret values are only for isolated local verification. Before a shared deployment, copy their filenames into a protected directory, replace every value, set `SECRET_DIR` in `docker/env/.env`, and restrict file permissions.
...
```bash
docker/scripts/compose.sh --profile '*' ps
docker/scripts/compose.sh --profile '*' logs --tail=200 SERVICE
make compose-down
```
