...
| D-037 | 已确认 | 新仓库采用 `backend / deploy / docker / docs / portal` 顶层目录 | 根 `pom.xml` 只作 Maven 聚合/版本管理；Java 服务和镜像定义进入 `backend/*`，部署支持进入 `deploy`，`docker` 只保留 Compose 入口，前端进入 `portal`，OpenAPI 只在 `docs/openapi` |
...
Compose 入口固定为 `docker/docker-compose.yml`；服务名或路径变化必须同步更新 Makefile、README、验证脚本和文档链接。容器镜像定义放在所属模块，其他部署支持文件放在 `deploy/`。
...
│   └── pnpm-lock.yaml
├── deploy/
│   ├── config/                      APISIX、Flink 和 OpenTelemetry 配置
│   ├── e2e/fixtures/                端到端验证输入
│   ├── env/.env.example             只含占位和说明
│   ├── scripts/                     bootstrap、health、E2E、backup/restore
│   ├── secrets/examples/            仅限本地的 Secret 占位值
...
- `docker` 只保留 Compose 入口；部署配置、脚本和验证数据统一放在 `deploy`，镜像定义由所属模块维护；`docs/openapi/openapi.yaml` 是公共 API 唯一契约源。
...
|---|---|---|---|
| P00 | `build(platform): establish project baseline` | 将最新 `docs/plan.md` 放入新仓库；完成 ADR、Maven 多模块、portal 外壳、OpenAPI、Makefile、CI、Java 21/pnpm 规范和目录清理 | JFOC |
...
下一步是新仓库 `/Users/dijkstra/project/06-shixi/Ontology` 的 P00：复制最新 `docs/plan.md`，建立 ADR、Maven/portal/OpenAPI/Makefile/CI 基线和正式目录。P00 完成全部门禁后提交 `build(platform): establish project baseline`，再进入 P01；不得一次领取多个 Phase 或把后续页面提前混入当前 commit。
