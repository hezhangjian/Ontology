...
Use `backend/`, `deploy/`, `docker/`, `docs/`, `examples/`, and `portal/` as the top-level implementation directories. The root Maven project only aggregates Java modules. Public HTTP contracts live only in `docs/openapi/openapi.yaml`. Keep `docker/` limited to `docker-compose.yml`; deployment support belongs in `deploy/` and image definitions belong to the module they build.
