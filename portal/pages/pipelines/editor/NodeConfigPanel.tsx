...
import { Input, InputNumber, Select, Switch } from '@/shared/components/forms';
import { Divider, Space } from '@/shared/components/layout';
...
import type { FieldSchema, Pipeline, PipelineNode, RuntimeSettings, ScheduleSettings } from '../types';
...
export default function NodeConfigPanel({ pipeline, node, linkTypes, objectTypes, connections, lookupAssets, onLookupAssetChange, onLookupConnectionChange, onNodeChange, onRuntimeChange, onScheduleChange }: Props) {
  ...
  const selectedSourceAssets = lookupAssets[String(config.connectionId ?? '')] ?? [];
  return <aside className="pipeline-config-panel">
    ...
    {node.type === 'SOURCE' && <>
      <ConfigLabel label="数据连接"><Select onChange={(connectionId) => {
        update({ assetId: undefined, connectionId });
        onLookupConnectionChange(connectionId);
      }} options={connections.filter((connection) => connection.status.startsWith('HEALTHY'))
        .map((connection) => ({ label: connection.name, value: connection.id }))} value={config.connectionId as string} /></ConfigLabel>
      <ConfigLabel label="文件、数据表或 Topic"><Select disabled={!config.connectionId} onChange={(assetId) => {
        update({ assetId });
        onLookupAssetChange(String(config.connectionId), assetId);
      }} options={selectedSourceAssets.filter((asset) => asset.assetType !== 'BUCKET' && asset.permissionStatus === 'READABLE')
        .map((asset) => ({ label: asset.name, value: asset.id }))} value={config.assetId as string} /></ConfigLabel>
      <ConfigLabel label="初始消费位置"><Select onChange={(offsetPolicy) => update({ offsetPolicy })} options={['EARLIEST', 'LATEST', 'TIMESTAMP', 'SPECIFIC_OFFSETS'].map((value) => ({ label: value, value }))} value={config.offsetPolicy as string ?? 'EARLIEST'} /></ConfigLabel>
      <Alert description="每个 Source 会编译为独立 Flink connector；可连接到不同分支或双输入 Join。" message="受控多源输入" showIcon type="info" />
    </>}
    {node.type === 'SELECT' && <FieldSelection
      fields={node.inputSchema}
      label="保留字段"
      onChange={(values) => update({ fields: values.map((field) => ({ source: field, target: field })) })}
      selected={Array.isArray(config.fields) ? (config.fields as Array<{ source: string }>).map((field) => field.source) : []}
    />}
    ...
    {node.type === 'DEDUPLICATE' && <><FieldSelection fields={node.inputSchema} label="去重键" maxHeight={190} onChange={(keys) => update({ keys })} selected={config.keys as string[] ?? []} /><ConfigLabel label="状态保留时间（毫秒）"><InputNumber min={1000} onChange={(stateTtlMs) => update({ stateTtlMs })} value={config.stateTtlMs as number ?? 86400000} /></ConfigLabel></>}
    {node.type === 'JOIN' && <>
      <Alert description="将两个上游分支都连入该节点；左右顺序按连线顺序固定在当前部署中。" message="双 DataStream 窗口关联" showIcon type="info" />
      <ConfigLabel label="左侧关联列"><Select onChange={(leftKey) => update({ leftKey })} options={availableFields} value={config.leftKey as string} /></ConfigLabel>
      <ConfigLabel label="右侧关联列"><Select onChange={(rightKey) => update({ rightKey })} options={availableFields} value={config.rightKey as string} /></ConfigLabel>
      <ConfigLabel label="保留哪些记录"><Select onChange={(joinType) => update({ joinType })} options={[
        { label: '全部匹配', value: 'INNER' },
        { label: '保留全部左侧', value: 'LEFT' },
        { label: '保留全部右侧', value: 'RIGHT' },
        { label: '保留两侧全部', value: 'FULL' },
      ]} value={config.joinType as string ?? 'LEFT'} /></ConfigLabel>
      <ConfigLabel label="关联窗口（毫秒）"><InputNumber min={1000} onChange={(windowSizeMs) => update({ windowSizeMs })} value={config.windowSizeMs as number ?? 60000} /></ConfigLabel>
      <ConfigLabel label="右侧同名列前缀"><Input onChange={(event) => update({ lookupPrefix: event.target.value })} value={String(config.lookupPrefix ?? 'right_')} /></ConfigLabel>
    </>}
    {node.type === 'WINDOW' && <><ConfigLabel label="窗口类型"><Select onChange={(windowType) => update({ windowType })} options={['TUMBLING', 'SLIDING', 'SESSION'].map((value) => ({ label: value, value }))} value={config.windowType as string ?? 'TUMBLING'} /></ConfigLabel><FieldSelection fields={node.inputSchema} label="分区字段" maxHeight={190} onChange={(groupBy) => update({ groupBy })} selected={config.groupBy as string[] ?? []} /><ConfigLabel label="窗口大小（毫秒）"><InputNumber min={1000} onChange={(windowSizeMs) => update({ windowSizeMs })} value={config.windowSizeMs as number ?? 60000} /></ConfigLabel>{config.windowType === 'SLIDING' && <ConfigLabel label="滑动步长（毫秒）"><InputNumber min={1000} onChange={(slideMs) => update({ slideMs })} value={config.slideMs as number ?? 30000} /></ConfigLabel>}{config.windowType === 'SESSION' && <ConfigLabel label="会话间隔（毫秒）"><InputNumber min={1000} onChange={(sessionGapMs) => update({ sessionGapMs })} value={config.sessionGapMs as number ?? 60000} /></ConfigLabel>}</>}
    {node.type === 'AGGREGATE' && <><FieldSelection fields={node.inputSchema} label="分组字段" maxHeight={190} onChange={(groupBy) => update({ groupBy })} selected={config.groupBy as string[] ?? []} /><ConfigLabel label="聚合方式"><Select onChange={(aggregation) => update({ aggregation })} options={['COUNT', 'SUM', 'AVG', 'MIN', 'MAX', 'COUNT_DISTINCT'].map((value) => ({ label: value, value }))} value={config.aggregation as string ?? 'COUNT'} /></ConfigLabel>{config.aggregation !== 'COUNT' && <ConfigLabel label="指标字段"><Select onChange={(field) => update({ field })} options={availableFields} value={config.field as string} /></ConfigLabel>}<ConfigLabel label="输出字段名"><Input onChange={(event) => update({ outputField: event.target.value })} value={String(config.outputField ?? String(config.aggregation ?? 'count').toLowerCase())} /></ConfigLabel></>}
...
function RuleTransform({ config, fields, update }: { config: Record<string, unknown>; fields: string[]; update: (values: Record<string, unknown>) => void }) {
  ...
  return <>
    <Alert message="规则以配置形式进入当前部署；原值可保留为审计字段。" showIcon type="info" />
...
function DatasetOutput({ config, fields, update }: { config: Record<string, unknown>; fields: FieldSchema[]; update: (values: Record<string, unknown>) => void }) {
  ...
  const selectedFields = explicit ? configured.map((item) => item.source).filter((field) => fieldNames.includes(field)) : fieldNames;
  ...
  return <><ConfigLabel label="数据集名称"><Input onChange={(event) => update({ datasetName: event.target.value })} placeholder="例如：员工 Token 明细" value={String(config.datasetName ?? '')} /></ConfigLabel><ConfigLabel label="Dataset ID"><Input onChange={(event) => update({ datasetId: event.target.value })} placeholder="例如：employee-token-detail" value={String(config.datasetId ?? '')} /></ConfigLabel><ConfigLabel label="说明"><Input.TextArea onChange={(event) => update({ description: event.target.value })} rows={2} value={String(config.description ?? '')} /></ConfigLabel><Divider>输出字段</Divider><FieldSelection fields={fields} label="物化字段" onChange={selectFields} selected={selectedFields} />{mappings.length > 0 && <section className="field-mapping"><header><strong>输出名称</strong><Text type="secondary">保持为空时沿用源字段名</Text></header><div className="field-mapping-list">{mappings.map((mapping)=><label className="field-mapping-row" key={mapping.source}><span title={mapping.source}>{mapping.source}</span><Input aria-label={`${mapping.source} 的输出名称`} onChange={(event)=>changeTarget(mapping.source,event.target.value)} placeholder={mapping.source} size="small" value={mapping.target}/></label>)}</div></section>}</>;
}

function ConfigLabel({ children, label }: { children: React.ReactNode; label: string }) {
  const child = children as React.ReactElement<{ 'aria-label'?: string }>;
  const control = React.isValidElement(child)
    ? React.cloneElement(child, {
      'aria-label': child.props['aria-label'] ?? label,
    })
    : children;
  return <label className="pipeline-config-field"><span>{label}</span>{control}</label>;
}
...
function ObjectMapping({ config, fields, objectTypes, update }: { config: Record<string, unknown>; fields: string[]; objectTypes: ObjectTypeDefinition[]; update: (values: Record<string, unknown>) => void }) {
  ...
  const chooseType = (objectTypeId: string) => {
    ...
    update({ objectTypeId, idFields: [], mappings: automaticMappings });
  };
  return <>
    ...
    <ConfigLabel label="哪些列共同组成唯一标识"><Select mode="multiple" onChange={(idFields) => update({ idFields })} options={fieldOptions} placeholder="可选择一列或多列" value={Array.isArray(config.idFields) ? config.idFields as string[] : []} /></ConfigLabel>
