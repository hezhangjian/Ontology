import { ArrowLeftOutlined, CloseCircleOutlined, RedoOutlined, ReloadOutlined } from '@/shared/icons';
import { Button, Tag } from '@/shared/components/actions';
import { Descriptions, Steps, Table, Timeline } from '@/shared/components/data';
import { Alert, Progress, Result, Spin, Statistic, message } from '@/shared/components/feedback';
import { Card, Col, Row, Space } from '@/shared/components/layout';
...
export default function PipelineRunPage({ navigate, pipelineId, runId }: { navigate: (path: string) => void; pipelineId: string; runId: string }) {
  ...
  async function retry() {
    try {
      const nextRun = await api.retry(runId);
      message.success('已创建新的重试 Run');
      navigate(`/data/pipelines/${pipelineId}/runs/${nextRun.id}`);
    } catch (cause) {
  ...
  async function replayDlq() {
    try {
      const nextRun = await api.replayDlq(runId);
      message.success('已创建新的 DLQ 重放 Run');
      navigate(`/data/pipelines/${pipelineId}/runs/${nextRun.id}`);
    } catch (cause) {
      message.error(cause instanceof Error ? cause.message : 'DLQ 重放失败');
    }
  }
  ...
  return <div className="pipeline-run-page">
    <div className="page-title-row"><Space align="start"><Button icon={<ArrowLeftOutlined />} onClick={() => navigate(`/data/pipelines/${pipelineId}/edit`)} type="text" /><div><Space><Title level={2}>{run.pipelineName}</Title><Tag color={statusColor(run.status)}>{run.status}</Tag></Space><Paragraph>Run {run.id}</Paragraph></div></Space><Space><Button icon={<ReloadOutlined />} onClick={() => void load()}>刷新</Button>{!terminal.includes(run.status) && <Button danger icon={<CloseCircleOutlined />} onClick={() => void action('取消请求已提交；已投影对象不会撤销', () => api.cancel(run.id))}>取消</Button>}{['FAILED', 'CANCELLED', 'DEGRADED'].includes(run.status) && <Button icon={<RedoOutlined />} onClick={() => void retry()}>重试</Button>}{run.status === 'DEGRADED' && <Button onClick={() => void replayDlq()}>重放 DLQ</Button>}</Space></div>
