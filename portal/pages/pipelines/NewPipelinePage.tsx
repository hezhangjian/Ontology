import { ArrowLeftOutlined } from '@/shared/icons';
...
export default function NewPipelinePage({  navigate }: { navigate: (path: string) => void }) {
  const connectionApi = useMemo(() => dataConnectionsApi(), []);
  ...
  async function submit(values: Record<string, unknown>) {
    setSubmitting(true);
    try {
      const result = await api.create({ ...values, template: 'FILE_BATCH', mode: 'BATCH' });
  ...
  return <div className="new-pipeline-page">
    ...
    <div className="new-pipeline-heading"><Title level={2}>创建数据处理管道</Title><Paragraph>先选择主数据，进入编辑器后可继续添加第二个 Source、Join、对象输出和关系输出。</Paragraph></div>
    ...
    {loading ? <Spin /> : <Form form={form} layout="vertical" onFinish={(values) => void submit(values)}>
      <section className="pipeline-create-section"><div className="pipeline-create-grid"><Form.Item label="任务名称" name="name" rules={[{ required: true, message: '请输入名称' }, { max: 240 }]}><Input placeholder="例如：业务数据整理" /></Form.Item><Form.Item className="wide-field" label="说明（可选）" name="description"><Input.TextArea maxLength={1000} rows={2} /></Form.Item><Form.Item label="数据连接" name="dataSourceId" rules={[{ required: true, message: '请选择数据连接' }]}><Select onChange={() => form.setFieldValue('sourceAssetId', undefined)} options={connections.filter((connection) => !['KAFKA', 'EXTERNAL_PULSAR'].includes(connection.type)).map((connection) => ({ label: connection.name, value: connection.id }))} placeholder="选择 CSV 或数据库连接" /></Form.Item><Form.Item label="主文件或数据表" name="sourceAssetId" rules={[{ required: true, message: '请选择文件或数据表' }]}><Select disabled={!selectedConnection} options={assets.map((asset) => ({ label: asset.name, value: asset.id }))} placeholder="选择要整理的数据" /></Form.Item></div><Text type="secondary">编辑器支持多源 DAG、双输入 Join、Dataset、对象和关系等多个输出。</Text></section>
      <div className="pipeline-create-actions"><Button onClick={() => navigate('/data/pipelines')}>取消</Button><Button htmlType="submit" loading={submitting} type="primary">下一步：配置输出</Button></div>
    </Form>}
  </div>;
}
