import { BranchesOutlined, EllipsisOutlined, PlusOutlined, ReloadOutlined, SearchOutlined } from '@/shared/icons';
import { Avatar, Button, Tag } from '@/shared/components/actions';
import { Table } from '@/shared/components/data';
import type { ColumnsType } from '@/shared/components/data';
import { Alert, Empty, message } from '@/shared/components/feedback';
import { Input, Select } from '@/shared/components/forms';
import { Flex, Space } from '@/shared/components/layout';
import { Dropdown, Modal, Tooltip } from '@/shared/components/overlays';
...
export default function PipelineListPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const columns: ColumnsType<Pipeline> = [
    ...
    { title: '', fixed: 'right', width: 56, render: (_, pipeline) => <Dropdown menu={{ items: [
      ...
      { key: 'pause', label: pipeline.lifecycle === 'PAUSED' ? '恢复' : '暂停', disabled: !['PUBLISHED', 'PAUSED'].includes(pipeline.lifecycle), onClick: () => void operation(pipeline.lifecycle === 'PAUSED' ? '已恢复' : '已暂停', () => pipeline.lifecycle === 'PAUSED' ? api.resume(pipeline.id) : api.pause(pipeline.id)) },
      { key: 'archive', label: '归档', disabled: pipeline.lifecycle === 'ARCHIVED' || ['LIVE', 'RUNNING'].includes(pipeline.runStatus), onClick: () => Modal.confirm({ title: `归档“${pipeline.name}”？`, content: '运行历史和血缘将保留。', onOk: () => operation('已归档', () => api.archive(pipeline.id)) }) },
      { key: 'delete', danger: true, disabled: pipeline.lifecycle !== 'ARCHIVED', label: pipeline.lifecycle === 'ARCHIVED' ? '永久删除' : '永久删除（请先归档）', onClick: () => Modal.confirm({ title: `永久删除“${pipeline.name}”？`, content: '运行记录、调度与映射将一并删除；若仍有关联 Dataset，平台会要求先删除 Dataset。此操作不可撤销。', okButtonProps: { danger: true }, okText: '永久删除', onOk: () => operation('已删除管道', () => api.delete(pipeline.id)) }) },
