import { ArrowLeftOutlined, EyeOutlined, PlayCircleOutlined, SaveOutlined } from '@/shared/icons';
import { Button, Tag } from '@/shared/components/actions';
import { Alert, Result, Spin, message } from '@/shared/components/feedback';
...
export default function PipelineEditorPage({  id, navigate }: Props) {
  const api = useMemo(() => pipelinesApi(), []);
  const datasetApi = useMemo(() => datasetsApi(), []);
  const explorerApi = useMemo(() => new ExplorerApi(), []);
  const ontologyApi = useMemo(() => modelingApi(), []);
  const connectionsApi = useMemo(() => dataConnectionsApi(), []);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [pipelineResponse, definitions, explorerHome, links, connectionPage] = await Promise.all([
      ...
      setPipeline(data); setNodeTypes(definitions); setObjectTypes(explorerHome.objectTypes); setLinkTypes(links);
      etagRef.current = Number(pipelineResponse.etag?.replaceAll('"', '') ?? 0);
  ...
  const saveDraft = useCallback(async () => {
    ...
    setSaveState('saving');
    try {
      const { data, etag } = await api.updateDraft(id, etagRef.current, { graph, runtime, schedule });
      etagRef.current = Number(etag?.replaceAll('"', '') ?? etagRef.current);
  ...
  function addNode(definition: NodeType) {
    ...
    const selectedDefinition = nodeTypes.find((node) => node.type === selectedNode?.type);
    if (definition.output && selectedNode && selectedDefinition?.output) {
      changeGraph({
        ...graph,
        nodes: graph.nodes.map((node) => node.id === selectedNode.id
          ? {
            ...node,
            config: {},
            inputSchema: [],
    ...
    const id = `${definition.type.toLowerCase()}-${randomUuid().slice(0, 8)}`;
    ...
    const edges = definition.type !== 'SOURCE' && selectedId && incoming.length === 1
      ? [...graph.edges.filter((edge) => edge.id !== incoming[0].id), { ...incoming[0], target: id }, { id: `edge-${randomUuid()}`, source: id, target: selectedId }]
      : graph.edges;
  ...
  function duplicateSelected() {
    ...
    if (!source) return;
    const id = `${source.type.toLowerCase()}-${randomUuid().slice(0, 8)}`;
  ...
  async function runPreview() {
    const nodeId = editor.selectedNodeId ?? graph.nodes.find((node) => ['DATASET_OUTPUT', 'LINK_OUTPUT', 'OBJECT_OUTPUT'].includes(node.type))?.id;
  ...
  async function finishAndRun() {
    try {
      ...
      if (!checked.valid) {
        ...
        message.error('请先补全右侧的输出节点和字段配置');
        return;
      }
      const datasetOutput = checked.normalizedGraph.nodes.find((node) => node.type === 'DATASET_OUTPUT');
      const projectionOutputs = checked.normalizedGraph.nodes.filter((node) => node.type === 'LINK_OUTPUT' || node.type === 'OBJECT_OUTPUT');
      if (datasetOutput && projectionOutputs.length === 0) {
        const dataset = await datasetApi.materialize(
          id,
          String(datasetOutput.config.datasetId ?? ''),
          String(datasetOutput.config.datasetName ?? ''),
          String(datasetOutput.config.description ?? ''),
        );
      ...
      message.success('对象与关系投影任务已提交');
      navigate(`/data/pipelines/${id}/runs/${run.id}`);
    } catch (cause) { message.error(cause instanceof Error ? cause.message : '生成管道输出失败'); }
  }
  ...
  const saveLabel = { conflict: '保存冲突', dirty: '待保存', saved: '已保存', saving: '保存中' }[saveState];
  return <div className="pipeline-editor-page">
    <header className="pipeline-editor-header"><Space><Button aria-label="返回管道列表" icon={<ArrowLeftOutlined />} onClick={() => navigate('/data/pipelines')} type="text" /><div><Space><Title level={4}>{pipeline.name}</Title><Tag>{pipeline.mode === 'BATCH' ? '批处理' : '实时'}</Tag></Space><Text className={`save-state save-state-${saveState}`}><SaveOutlined /> {saveLabel}</Text></div></Space><Space><Button icon={<EyeOutlined />} onClick={() => void runPreview()}>预览数据</Button><Button icon={<PlayCircleOutlined />} onClick={() => void finishAndRun()} type="primary">生成输出</Button></Space></header>
