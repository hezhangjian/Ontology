...
export default function DatasetEditorPage({  id, navigate }: Props) {
  ...
  useEffect(() => {
    if (!id) return;
    let active = true;
    void api.getForEdit(id).then(async ({ data: dataset, etag: currentEtag }) => {
  ...
  const save = async () => {
    ...
    if (id && !etag) {
      void message.error('未能获取 Dataset 的最新版本，请刷新页面后重试');
      return;
    }
    ...
    try {
      const value = id
        ? await api.update(id, etag, manual ? { description, name, rows: savedRows } : { description, name })
