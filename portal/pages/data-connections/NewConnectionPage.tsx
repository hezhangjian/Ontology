...
import { Descriptions, Steps } from '@/shared/components/data';
import { Alert, Spin, message } from '@/shared/components/feedback';
import { Checkbox, Form, Input, InputNumber, Radio, Select } from '@/shared/components/forms';
import { Card, Divider, Space } from '@/shared/components/layout';
...
export default function NewConnectionPage({ navigate }: Props) {
  const api = useMemo(() => dataConnectionsApi(), []);
  ...
  async function create() {
    ...
    try {
      const source = await api.create({
        name: values.name.trim(), description: values.description, type,
  ...
  const cancel = () => {
    ...
    Modal.confirm({
      title: '放弃新建连接？',
      content: '当前向导内容尚未保存，离开后本次填写将丢失。',
      okButtonProps: { danger: true },
      okText: '放弃创建',
      onOk: () => navigate('/data/connections'),
    });
  };
  const values = form.getFieldsValue(true);
      ...
      <Form<FormValues> form={form} initialValues={{ credentialMode: 'MANAGED', timeoutSeconds: 15 }} layout="vertical" onValuesChange={() => { setDirty(true); setTestResult(undefined); }}>
        <div className="wizard-body">
          ...
          {step === 1 && type && type !== 'LOCAL_CSV' && <Card className="wizard-card" title={`${sourceTypes.find((item) => item.value === type)?.title} 配置`}>
            <div className="form-grid"><Form.Item label="连接名称" name="name" rules={[{ required: true, whitespace: true, message: '请输入连接名称' }, { max: 160 }]}><Input placeholder="例如：生产订单数据" /></Form.Item></div>
            ...
            <Divider orientation="left">凭据与证书</Divider><Form.Item label="凭据来源" name="credentialMode"><Radio.Group options={[{ label: '新建平台凭据', value: 'MANAGED' }, { label: '使用已有凭据', value: 'EXISTING' }, { label: '预置 Docker Secret', value: 'FILE' }]} /></Form.Item>
