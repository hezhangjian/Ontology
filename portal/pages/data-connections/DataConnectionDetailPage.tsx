...
import { Alert, Empty, Result, Spin, message } from '@/shared/components/feedback';
import { Form, Input, Radio, Select } from '@/shared/components/forms';
import { Card, Space } from '@/shared/components/layout';
import { Tabs } from '@/shared/components/navigation';
import { Drawer, Dropdown, Modal } from '@/shared/components/overlays';
...
export default function DataConnectionDetailPage({ assetId, id, navigate }: Props) {
  ...
  return (
    <div className="connection-detail-page">
      ...
      <RotateCredentialModal id={id} onClose={() => setRotateOpen(false)} onSuccess={async () => { setRotateOpen(false); await load(); }} open={rotateOpen} source={source} />
    </div>
  );
}
...
function RotateCredentialModal({ id, onClose, onSuccess, open, source }: {
    id: string;
  onClose: () => void;
  ...
  useEffect(() => {
    if (!open) return;
    form.resetFields();
    form.setFieldsValue({ credentialMode: 'FILE', credentialName: `${source.name} 轮换凭据` });
    void api.listCredentials().then(setCredentials).catch(() => setCredentials([]));
  }, [api, form, open, source.name]);
    ...
    <Form form={form} layout="vertical">
      <Form.Item label="凭据来源" name="credentialMode">
        <Radio.Group>
          ...
          <Radio value="FILE">预置 Docker Secret</Radio>
        </Radio.Group>
      </Form.Item>
