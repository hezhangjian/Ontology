...
import { Alert, Result, Skeleton, message } from '@/shared/components/feedback';
import { Checkbox, Form, Input, InputNumber, Select } from '@/shared/components/forms';
...
export default function EditConnectionPage({  id, navigate }: Props) {
  ...
  useEffect(() => {
    void api.getForEdit(id).then(({ data: value, etag: currentEtag }) => {
      setSource(value);
      setEtag(Number(currentEtag?.replaceAll('"', '') ?? 0));
      form.setFieldsValue({
        ...value.config,
        name: value.name,
  ...
  const leave = () => {
    ...
    Modal.confirm({
      title: '放弃未保存的修改？',
      content: '编辑内容尚未保存，离开后本次修改将丢失。',
      okButtonProps: { danger: true },
      okText: '放弃修改',
      onOk: () => navigate(`/data/connections/${id}?tab=settings`),
    });
  };
  const save = async () => {
    if (!source) return;
    ...
    setSaving(true);
    try {
      const updated = await api.update(id, etag, {
        name: values.name,
      ...
      setSource(updated.data);
      setEtag(Number(updated.etag?.replaceAll('"', '') ?? etag));
      setDirty(false);
