...
function FormBase<T>({ children, form: providedForm, initialValues = {}, layout, onFinish, onValuesChange, requiredMark, ...props }: AnyProps) {
  const [internalForm] = useState<FormApi<T>>(() => createForm<T>());
  ...
  return <FormContext.Provider value={{ form, onValuesChange }}>
    <HeroForm {...props} onSubmit={(event) => { event.preventDefault(); void form.validateFields().then(onFinish); }}>{children}</HeroForm>
...
function FormItem({ children, className, extra, hidden, initialValue, label, name, required, rules = [], style, valuePropName }: AnyProps) {
  ...
  if (name) {
    ...
    if (form && form.getFieldValue(name) === undefined && initialValue !== undefined && form.__values) form.__values[name] = initialValue;
  }
  ...
  const child = React.isValidElement(children) && name ? React.cloneElement(children as React.ReactElement<any>, {
    'aria-describedby': error ? errorId : (children as any).props['aria-describedby'],
  ...
  return <div className={cx('form-field mb-3.5 grid gap-1.5', error && 'has-error', className)} style={style}>
    {label && <label className="text-sm font-semibold text-gray-700" htmlFor={fieldId}>{label}{requiredField && <span aria-hidden className="form-required text-danger"> *</span>}</label>}
    {child}
    {extra && <small className="text-gray-500">{extra}</small>}
    {error && <small className="form-error text-danger" id={errorId} role="alert">{error}</small>}
  </div>;
}
