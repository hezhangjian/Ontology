...
export function conversationApi() {
  ...
  const base = () => `/v1/ontologies/${activeOntologyId()}/agents/${agentId}/conversations`;
  ...
  return {
    confirmAction: (conversationId: string, actionId: string, previewToken: string) => request<Record<string, unknown>>(`/${conversationId}/action-confirmations`, { method: 'POST', headers: { 'Idempotency-Key': `conversation:${conversationId}:${randomUuid()}` }, body: JSON.stringify({ actionTypeId: actionId, previewToken }) }),
    confirmRule: (conversationId: string, change: Record<string, unknown>) => request<Record<string, unknown>>(`/${conversationId}/confirm-rule-transform`, { method: 'POST', body: JSON.stringify(change) }),
