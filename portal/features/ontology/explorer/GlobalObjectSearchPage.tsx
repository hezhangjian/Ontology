...
export default function GlobalObjectSearchPage({  navigate }: { navigate: (path: string) => void }) {
  ...
  return <div className="explorer-page global-search-page"><div className="explorer-page-heading"><Typography.Title level={2}>全局对象搜索</Typography.Title><Typography.Paragraph>在当前权限范围内查找对象实例、对象类型和已保存的工作内容。</Typography.Paragraph></div><Input.Search aria-label="全局对象搜索" enterButton onChange={(event) => setQuery(event.target.value)} onSearch={run} prefix={<SearchOutlined />} size="large" value={query} />{error && <Alert message={error} showIcon type="error" />}{!result && !error ? <Skeleton active /> : <Tabs className="search-results-tabs" items={[
