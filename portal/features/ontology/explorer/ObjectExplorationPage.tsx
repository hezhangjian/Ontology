...
import { Form, Input, Select } from "@/shared/components/forms";
import { Card, Space } from "@/shared/components/layout";
import { Tabs } from "@/shared/components/navigation";
import { Drawer, Modal } from "@/shared/components/overlays";
...
export default function ObjectExplorationPage({
  ...
  const [perspective, setPerspective] = useState("TABLE");
  ...
  const [saveMode, setSaveMode] = useState<
    "exploration" | "list" | "export" | null
  >(null);
  const [form] = Form.useForm();
  const [compare, setCompare] = useState<Record<string, unknown>>();
  const [relationGraph, setRelationGraph] = useState<RelationGraph>();
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setHome(loaded);
        if (explorationId) {
          const saved = await api.exploration(explorationId);
          setPerspective(saved.perspective);
          await run(saved.query);
        } else {
  ...
  }, [api, explorationId, objectTypeId, run]);
  ...
  const submitSave = async () => {
    ...
    try {
      if (saveMode === "exploration") {
        const saved = await api.createExploration({
          ...values,
          objectTypeId: type.id,
          query,
        ...
        notice.success(`探索“${saved.name}”已保存`);
      }
      if (saveMode === "list") {
        ...
        notice.success(`静态清单已保存 ${list.itemCount} 个对象引用`);
      }
      if (saveMode === "export") {
        ...
        notice.success(`导出完成：${job.rowCount} 行，24 小时后过期`);
      }
      setSaveMode(null);
      form.resetFields();
    } catch (cause) {
      notice.error(cause instanceof Error ? cause.message : "操作失败");
    }
  };
  const runCompare = async () => {
    if (!query || selected.length < 2 || selected.length > 5)
      return notice.warning("请选择 2—5 个对象");
    try {
      setCompare(await api.compare(query.objectTypeId, selected));
    } catch (cause) {
      notice.error(cause instanceof Error ? cause.message : "比较失败");
    }
  };
  const changePerspective = async (next: string) => {
    setPerspective(next);
    if (next !== "GRAPH" || !page) return;
    const roots = (
      selected.length
        ? page.items.filter((item) => selected.includes(item.objectId))
    ...
    try {
      setRelationGraph(await api.relationGraph(roots));
    } catch (cause) {
      notice.error(cause instanceof Error ? cause.message : "关系图加载失败");
    } finally {
  ...
  return (
    <div className="object-explorer-page">
      ...
      <div className="explorer-toolbar">
        <div>
          <Space>
            ...
            <Tag color="blue">{type?.maturity}</Tag>
          </Space>
        ...
        <Space wrap>
          <Button
            icon={<SaveOutlined />}
            onClick={() => setSaveMode("exploration")}
          >
            保存探索
          </Button>
          <Button
          ...
          <div className="object-set-summary">
            ...
            {page?.queryFingerprint && (
              <Typography.Text code>
                {page.queryFingerprint.slice(0, 12)}…
              </Typography.Text>
            )}
          </div>
          <Tabs
            activeKey={perspective}
            onChange={(value) => void changePerspective(value)}
            items={[
              {
                key: "TABLE",
                label: (
              ...
              },
              {
                ...
                children: (
                  <ObjectCardView items={page?.items ?? []} open={setPanel} />
                ),
              },
              {
                key: "ANALYSIS",
                label: (
                  <span>
                    <BarChartOutlined /> 快速分析
                  </span>
                ),
                children: page && (
                  <QuickAnalysisView onFilter={addFilter} page={page} />
                ),
              },
              {
                key: "GRAPH",
      ...
      <Modal
        ...
        open={Boolean(saveMode)}
        title={
          saveMode === "exploration"
            ? "保存动态 Exploration"
            : saveMode === "list"
              ? "保存静态对象清单"
              : "创建异步安全导出"
              ...
              >
                <Select
                  options={[
                    ...
                    { value: "SHARED", label: "共享（不提升对象权限）" },
                  ]}
                />
              </Form.Item>
            </>
          )}
          {saveMode === "list" && !selected.length && (
            <Alert
              message="请先选择对象；清单只保存引用，不保存正文。"
              type="warning"
            />
          )}
          {saveMode === "export" && (
            <>
              <Form.Item initialValue="CSV" label="格式" name="format">
                <Select
                  options={[
                    { value: "CSV", label: "CSV" },
                    { value: "IDS", label: "仅对象 ID" },
                  ]}
                />
              </Form.Item>
              <Alert
                message={`将导出${selected.length ? `选中的 ${selected.length} 个对象` : "当前结果"}；敏感/无权字段不会写入文件，文件 24 小时过期。`}
                type="info"
              />
            </>
          )}
      ...
      <Drawer
        ...
        width={760}
      >
        {compare ? <pre>{JSON.stringify(compare, null, 2)}</pre> : <Empty />}
      </Drawer>
    </div>
  );
}
