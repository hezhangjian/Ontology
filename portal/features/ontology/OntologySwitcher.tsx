import { DownOutlined, PlusOutlined, SettingOutlined } from '@/shared/icons';
...
export default function OntologySwitcher({ collapsed, onChanged }: { collapsed: boolean; onChanged: () => void }) {
  ...
  const collapsedLabel = active?.name.trim().slice(0, 1) || '-';
  return <>
    <Dropdown menu={{ items: menuItems, onClick: choose }} placement="bottomLeft" trigger={['click']}>
      <Button
        ...
        title={label}
      >
        {collapsed && <span aria-hidden className="ontology-collapsed-name">{collapsedLabel}</span>}
