import { DashboardOutlined, DeleteOutlined, EditOutlined, EyeOutlined, PlusOutlined, SearchOutlined } from '@/shared/icons';
...
export default function DashboardListPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <div className="dashboard-list-page collection-page">{context}<div className="page-title-row"><div><Typography.Title level={2}>分析看板</Typography.Title><Typography.Paragraph>新建看板后，自行选择 Dataset 或业务对象、图表类型、维度和指标。</Typography.Paragraph></div><Button icon={<PlusOutlined />} onClick={() => setOpen(true)} type="primary">新建看板</Button></div>
    <div className="collection-toolbar"><Input.Search allowClear onSearch={() => void load()} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索看板" prefix={<SearchOutlined />} value={keyword} /><span>{items.length} 个看板</span></div>
    <Table dataSource={items} locale={{ emptyText: <Empty description={<><div className="empty-state-icon"><DashboardOutlined /></div><Typography.Title level={4}>还没有分析看板</Typography.Title><Typography.Paragraph>创建看板后，可组合指标、图表和业务对象视图。</Typography.Paragraph><Button icon={<PlusOutlined />} onClick={() => setOpen(true)} type="primary">新建第一个看板</Button></>} /> }} pagination={{ pageSize: 20 }} rowKey="id" size="small" columns={[
