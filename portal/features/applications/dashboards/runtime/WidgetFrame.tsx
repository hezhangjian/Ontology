...
import { Alert, Empty, Spin } from '@/shared/components/feedback';
...
function chart(type: string, data: Array<{ label: string; series: string; value: number }>) {
  if (type === 'PIE' || type === 'DONUT') {
    return <EChart option={{ tooltip: { trigger: 'item' }, series: [{ type: 'pie', radius: type === 'DONUT' ? ['48%', '72%'] : '72%', data: data.map((item) => ({ name: item.label, value: item.value })) }] }} />;
  }
  const seriesNames = [...new Set(data.map((item) => item.series || 'value'))];
  const labels = [...new Set(data.map((item) => item.label))];
  return <EChart option={{
    legend: seriesNames.length > 1 ? {} : undefined,
    ...
    series: seriesNames.map((name) => ({
      ...
      data: labels.map((label) => data.find((item) => item.label === label && (item.series || 'value') === name)?.value ?? 0),
    })),
  }} />;
}
