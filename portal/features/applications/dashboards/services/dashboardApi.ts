...
import type { DashboardBatchResult, DashboardDefinition, DashboardDetail, DashboardDraft, DashboardPlan, DashboardSummary, DashboardValidation } from '../types';
...
export class DashboardApi {
  ...
  publish = (id: string, releaseNotes = '') => this.request<DashboardDetail>(`/dashboards/${id}/publish`, { method: 'POST', body: JSON.stringify({ releaseNotes }) });
  plan = (id: string) => this.request<DashboardPlan>(`/dashboards/${id}/query-plan`);
  execute = (planId: string, pageId: string, widgetIds: string[], filters: Record<string, unknown>, refreshId = randomUuid()) => this.request<DashboardBatchResult>(`/dashboard-query-plans/${planId}/widgets:batch`, { method: 'POST', body: JSON.stringify({ pageId, widgetIds, filters, refreshId }) });
}
