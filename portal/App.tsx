...
import { Divider, Layout, Space } from '@/shared/components/layout';
...
const AiAssistedDevelopmentDashboardPage = lazy(() => import('./app/effective/AiAssistedDevelopmentDashboardPage'));
...
const DataAiOperationsDashboardPage = lazy(() => import('./app/quality/DataAiOperationsDashboardPage'));
...
const SavedResourcesPage = lazy(() => import('./features/ontology/explorer/SavedResourcesPage'));
...
function App() {
  ...
  const navigation = useMemo<MenuProps['items']>(() => [
    ...
    { key: 'ontology-group', type: 'group', label: '本体', children: [{ key: '/ontology', icon: <DeploymentUnitOutlined />, label: '本体管理' }, { key: '/ontology/explorer', icon: <SearchOutlined />, label: '对象探索' }] },
    {
      ...
      children: [
        ...(ontologyId === 'dataai' ? [
          { key: '/apps/ai-assisted-development', icon: <ExperimentOutlined />, label: 'AI辅助研发看板' },
          { key: '/apps/data-ai-operations', icon: <DashboardOutlined />, label: 'Data&AI运营看板' },
        ] : []),
  ...
  const crumbs = path.startsWith('/ontology/explorer')
    ...
    ? ['AIP', '对话中心']
    : path.startsWith('/apps/ai-assisted-development')
    ? ['应用', 'AI辅助研发看板']
    : path.startsWith('/apps/data-ai-operations')
    ? ['应用', 'Data&AI运营看板']
  ...
  const selectedNavigationKey = path.startsWith('/aip/conversations') ? '/aip/conversations' : path.startsWith('/ontology/explorer') ? '/ontology/explorer' : path.startsWith('/ontology') ? '/ontology' : path.startsWith('/apps/ai-assisted-development') ? '/apps/ai-assisted-development' : path.startsWith('/apps/data-ai-operations') ? '/apps/data-ai-operations' : path.startsWith('/apps/dashboards') ? '/apps/dashboards' : path.startsWith('/data/connections') ? '/data/connections' : path.startsWith('/data/pipelines') ? '/data/pipelines' : path.startsWith('/data/datasets') ? '/data/datasets' : path;
      ...
      <Sider className="app-sider" collapsed={collapsed} collapsedWidth={72} trigger={null} width={238}>
        <div className="brand">
          <div className="brand-identity" aria-label="Ontology Platform">
            ...
            {!collapsed && <div className="brand-copy"><div className="brand-name">Ontology</div><div className="brand-caption">Data Intelligence</div></div>}
          </div>
        ...
        <div className="sider-footer"><Divider /><Button aria-label={collapsed ? '展开导航' : '折叠导航'} className="collapse-button" icon={collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />} onClick={() => setCollapsed((current) => !current)} type="text">{!collapsed && '收起导航'}</Button></div>
      </Sider>
      <Layout>
...
function RouteContent({ location, navigate, ontologyId }: { location: string; navigate: (path: string) => void; ontologyId: string }) {
  ...
  if (path === '/apps/ai-assisted-development' || path === '/apps/data-ai-operations') {
    if (ontologyId !== 'dataai') return <Result extra={<Button onClick={() => navigate('/apps/dashboards')}>返回应用</Button>} status="404" title="页面不存在" />;
    return path === '/apps/ai-assisted-development'
      ? <AiAssistedDevelopmentDashboardPage />
      : <DataAiOperationsDashboardPage />;
  }
  ...
  if (path.startsWith('/ontology/explorer')) {
    const page = explorerRoute({ location, navigate, path });
...
function explorerRoute({ location, navigate, path }: { location: string; navigate: (path: string) => void; path: string }) {
  ...
  if (path === '/ontology/explorer') {
    ...
    if (section === 'explorations') return <SavedResourcesPage kind="explorations" navigate={navigate} />;
    if (section === 'lists') return <SavedResourcesPage kind="lists" navigate={navigate} />;
    return <ExplorerHomePage navigate={navigate} />;
  }
  const exploration = path.match(/^\/ontology\/explorer\/explorations\/([^/]+)$/);
  if (exploration) return <ObjectExplorationPage explorationId={exploration[1]} navigate={navigate} />;
  const list = path.match(/^\/ontology\/explorer\/lists\/([^/]+)$/);
  if (list) return <SavedResourcesPage kind="lists" navigate={navigate} />;
