...
function ObjectTypeWizard({ accessToken, displayName, navigate, userId }: { accessToken: string; displayName: string; navigate: (path: string) => void; userId: string }) {
  ...
  return <div className="ontology-editor-page"><Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/ontology/object-types')} type="text">返回对象类型</Button><div className="ontology-editor-heading"><Title level={2}>创建对象类型</Title></div>
    <Form form={form} layout="vertical" initialValues={{ apiName: generatedObjectName }}>
      <Card className="ontology-wizard-card"><Row gutter={18}><Col span={12}><Form.Item label="对象类型名称" name="displayName" rules={[{ required: true }]}><Input onChange={(event) => form.setFieldValue('apiName', automaticApiName(event.target.value, generatedObjectName))} placeholder="例如：人员、小组、设备" /></Form.Item></Col><Form.Item hidden name="apiName"><Input /></Form.Item><Col span={12}><Form.Item label="说明（可选）" name="description"><Input /></Form.Item></Col></Row>
    ...
    </Form><div className="ontology-wizard-actions"><Button loading={saving} onClick={() => void save()} type="primary">创建对象类型</Button></div>
  </div>;
}
...
function GenericResourceEditor({ accessToken, displayName, kind, navigate, userId }: { accessToken: string; displayName: string; kind: ResourceKind; navigate: (path: string) => void; userId: string }) {
  const api = useMemo(() => modelingApi(accessToken), [accessToken]); const [form] = Form.useForm(); const [objects, setObjects] = useState<OntologyResource[]>([]); const [links, setLinks] = useState<OntologyResource[]>([]); const [saving, setSaving] = useState(false);
  const generatedApiName = useMemo(() => `${kind.replace('_', '')}${Date.now().toString(36)}`, [kind]);
  useEffect(() => { void Promise.all([api.listResources('OBJECT_TYPE').then(setObjects), api.listResources('LINK_TYPE').then(setLinks)]); }, []);
  async function save() { const values = await form.validateFields(); setSaving(true); try { const target = values.targetObjectTypeId || values.leftObjectTypeId; const targetObject = objects.find((item) => item.id === target); const firstProperty = targetObject?.properties[0]; const body: Record<string, unknown> = { ...values, ownerId: userId, ownerName: displayName, maturity: 'ACTIVE', promoted: true, tags: [] };
      ...
      if (kind === 'ACTION') {
        const approvalParameter = values.approvalPolicy === 'CONDITIONAL'
          ? [{ apiName: 'requiresApproval', displayName: '是否触发审批', valueType: 'BOOLEAN', required: true, sensitive: false }]
          : [];
        const parameters = ['CREATE', 'UPDATE'].includes(values.operation)
          ? [{ apiName: 'value', displayName: '属性值', valueType: 'STRING', required: true, sensitive: false }, ...approvalParameter]
          : ['LINK', 'UNLINK'].includes(values.operation)
            ? [{ apiName: 'sourceObjectId', displayName: '起点对象 ID', valueType: 'STRING', required: true, sensitive: false },
              { apiName: 'targetObjectId', displayName: '终点对象 ID', valueType: 'STRING', required: true, sensitive: false }, ...approvalParameter]
            : approvalParameter;
        const rules = ['CREATE', 'UPDATE'].includes(values.operation)
          ? firstProperty ? [{ operation: 'SET_PROPERTY', targetPropertyId: firstProperty.id, valueFrom: 'value' }] : []
          : ['LINK', 'UNLINK'].includes(values.operation)
            ? [{ operation: values.operation, relationTypeId: values.relationTypeId,
              sourceObjectIdFrom: 'sourceObjectId', targetObjectIdFrom: 'targetObjectId' }]
            : [];
        Object.assign(body, { operation: values.operation, approvalPolicy: values.approvalPolicy, parameters, rules,
          submitCondition: values.approvalPolicy === 'CONDITIONAL'
            ? { approvalWhen: { operator: 'EQUALS', parameter: 'requiresApproval', value: true } } : {} });
      }
  ...
  const linkOptions = links.filter((item) => item.lifecycle === 'PUBLISHED')
    .map((item) => ({ label: item.displayName, value: item.id }));
  const actionOperation = Form.useWatch('operation', form);
  const sourceObjectId = Form.useWatch('leftObjectTypeId', form);
  const sourceProperties = objects.find((item) => item.id === sourceObjectId)?.properties ?? [];
  return <div className="ontology-editor-page"><Button icon={<ArrowLeftOutlined />} onClick={() => navigate(`/ontology/${resourcePath(kind, '').split('/')[2]}`)} type="text">返回{kindTitle[kind]}</Button><div className="ontology-editor-heading"><Title level={2}>新建{kindTitle[kind]}</Title></div><Card><Form form={form} layout="vertical" initialValues={{ apiName: generatedApiName, cardinality: 'N:1', operation: 'UPDATE', approvalPolicy: 'NONE', outputType: 'TABLE', timeoutMs: 5000 }}><Row gutter={18}><Col span={12}><Form.Item label="名称" name="displayName" rules={[{ required: true }]}><Input onChange={(event) => form.setFieldValue('apiName', automaticApiName(event.target.value, generatedApiName))} /></Form.Item></Col><Form.Item hidden name="apiName"><Input /></Form.Item><Col span={12}><Form.Item label="说明（可选）" name="description"><Input /></Form.Item></Col>{kind === 'LINK_TYPE' && <><Col span={8}><Form.Item label="起点对象类型" name="leftObjectTypeId" rules={[{ required: true }]}><Select options={objectOptions} /></Form.Item></Col><Col span={8}><Form.Item label="目标对象类型" name="rightObjectTypeId" rules={[{ required: true }]}><Select options={objectOptions} /></Form.Item></Col><Col span={8}><Form.Item label="关联数量" name="cardinality"><Select options={[{value:'1:1',label:'一对一'},{value:'1:N',label:'一对多'},{value:'N:1',label:'多对一'},{value:'N:M',label:'多对多'}]} /></Form.Item></Col><Col span={12}><Form.Item extra="例如选择人员.group_id；该值与小组主键相同的实例会形成关系边。" label="通过起点对象的哪个字段关联" name="sourcePropertyId" rules={[{ required: true, message: '请选择实例关联字段' }]}><Select disabled={!sourceObjectId} options={sourceProperties.map((property) => ({ label: `${property.displayName} (${property.apiName})`, value: property.id }))} placeholder="选择外键字段" /></Form.Item></Col></>}{kind === 'INTERFACE' && <Col span={12}><Form.Item label="适用对象类型" name="leftObjectTypeId" rules={[{ required: true }]}><Select options={objectOptions} /></Form.Item></Col>}{kind === 'ACTION' && <><Col span={8}><Form.Item label="目标对象类型" name="targetObjectTypeId" rules={[{ required: true }]}><Select options={objectOptions} /></Form.Item></Col><Col span={8}><Form.Item label="操作类型" name="operation"><Select options={['CREATE', 'UPDATE', 'RETIRE', 'LINK', 'UNLINK'].map((value) => ({ value }))} /></Form.Item></Col><Col span={8}><Form.Item label="是否需要审批" name="approvalPolicy"><Select options={[{value:'NONE',label:'不需要'},{value:'ALWAYS',label:'需要'},{value:'CONDITIONAL',label:'按条件'}]} /></Form.Item></Col>{['LINK', 'UNLINK'].includes(actionOperation) && <Col span={12}><Form.Item label="关系类型" name="relationTypeId" rules={[{ required: true }]}><Select options={linkOptions} /></Form.Item></Col>}</>}{kind === 'FUNCTION' && <><Col span={8}><Form.Item label="使用哪个对象类型" name="targetObjectTypeId" rules={[{ required: true }]}><Select options={objectOptions} /></Form.Item></Col><Col span={8}><Form.Item label="输出形式" name="outputType"><Select options={['SCALAR', 'OBJECT_SET', 'TABLE', 'BOOLEAN'].map((value) => ({ value }))} /></Form.Item></Col><Col span={8}><Form.Item label="超时 (ms)" name="timeoutMs"><InputNumber max={30000} min={100} style={{ width: '100%' }} /></Form.Item></Col></>}</Row><Alert message={kind === 'ACTION' ? 'Action 将统一经过 Preview、条件校验、可选审批、幂等执行和投影写回。' : '关系类型定义结构；具体实例关系由外键字段或管道投影生成，可在对象探索的“关系”页签查看。'} showIcon type="info" /><div className="edit-actions"><Button onClick={() => navigate(`/ontology/${resourcePath(kind, '').split('/')[2]}`)}>取消</Button><Button loading={saving} onClick={() => void save()} type="primary">创建{kindTitle[kind]}</Button></div></Form></Card></div>;
}
