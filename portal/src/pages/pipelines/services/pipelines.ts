...
export function pipelinesApi(accessToken: string) {
  ...
  return {
    ...
    publish: (id: string, proposalId?: string, startAfterPublish = false) => request<PipelineVersion>(`/pipelines/${id}/versions`, { method: 'POST', body: JSON.stringify({ acknowledgeWarnings: true, proposalId, startAfterPublish }) }),
    ...
    run: (id: string) => request<PipelineRun>(`/pipelines/${id}/runs`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({}) }),
