...
export default function PipelineListPage({ accessToken, navigate }: { accessToken: string; isAdmin: boolean; navigate: (path: string) => void }) {
  ...
  return <div className="pipelines-page">
    <div className="page-title-row"><div><Title level={2}>管道构建</Title><Paragraph>使用统一 Pipeline IR 编排批处理与流处理，Flink 是唯一计算引擎。</Paragraph></div><Space><Button icon={<ReloadOutlined />} onClick={() => void load()}>刷新</Button><Button icon={<PlusOutlined />} onClick={() => navigate('/data/pipelines/new')} type="primary">新建管道</Button></Space></div>
