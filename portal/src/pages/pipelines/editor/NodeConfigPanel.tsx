import { Alert, Divider, Input, InputNumber, Select, Space, Switch, Typography } from 'antd';
...
export default function NodeConfigPanel({ pipeline, node, linkTypes, objectTypes, connections, lookupAssets, onLookupAssetChange, onLookupConnectionChange, onNodeChange, onRuntimeChange, onScheduleChange }: Props) {
  ...
  return <aside className="pipeline-config-panel">
    ...
    {node.type === 'DEDUPLICATE' && <><ConfigLabel label="去重键"><Select mode="multiple" onChange={(keys) => update({ keys })} options={availableFields} value={config.keys as string[] ?? []} /></ConfigLabel><ConfigLabel label="状态保留时间（毫秒）"><InputNumber min={1000} onChange={(stateTtlMs) => update({ stateTtlMs })} value={config.stateTtlMs as number ?? 86400000} /></ConfigLabel></>}
    {node.type === 'JOIN' && <>
      <Alert description="将两个上游分支都连入该节点；左右顺序按连线顺序固定在发布版本中。" message="双 DataStream 窗口关联" showIcon type="info" />
