...
images="${APISIX_IMAGE} ${FLINK_BASE_IMAGE} ${HUGEGRAPH_IMAGE} ${JAVA_IMAGE} ${KEYCLOAK_IMAGE} ${MAVEN_IMAGE} ${MINIO_IMAGE} ${MINIO_MC_IMAGE} ${NGINX_IMAGE} ${NODE_IMAGE} ${OPENSEARCH_IMAGE} ${OTEL_COLLECTOR_IMAGE} ${POSTGRES_IMAGE} ${PULSAR_IMAGE} ${SKYWALKING_OAP_IMAGE} ${SKYWALKING_UI_IMAGE}"
