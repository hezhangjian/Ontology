...
class PipelineControllerTest {
    ...
    @Test
    void viewerCannotReadAndBuilderCanListPipelines() throws Exception {
        ...
        mvc.perform(get("/v1/pipelines").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_Viewer"))))
                .andExpect(status().isForbidden());
        mvc.perform(get("/v1/pipelines").with(jwt().jwt(token -> token.subject("builder").claim("name", "平台构建者"))
                        .authorities(new SimpleGrantedAuthority("ROLE_Builder"))))
                .andExpect(status().isOk());
    }

    @Test
    void onlyAdminCanResetOffsets() throws Exception {
        ...
        mvc.perform(post("/v1/pipelines/" + id + "/reset-offsets").contentType(MediaType.APPLICATION_JSON).content(body)
                        .with(jwt().jwt(token -> token.subject("builder")).authorities(new SimpleGrantedAuthority("ROLE_Builder"))))
                .andExpect(status().isForbidden());
        mvc.perform(post("/v1/pipelines/" + id + "/reset-offsets").contentType(MediaType.APPLICATION_JSON).content(body)
                        .with(jwt().jwt(token -> token.subject("admin").claim("name", "管理员"))
