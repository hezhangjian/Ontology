...
`docker/versions.env` is the executable lock manifest. External runtime and build images use an exact tag plus OCI index digest. `docker/scripts/compatibility-check.sh` then verifies that Docker resolved each lock to an image matching the current engine architecture.
