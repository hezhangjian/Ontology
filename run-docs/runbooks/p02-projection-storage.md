...
```bash
docker/scripts/compose.sh --profile '*' logs --tail=200 projection-worker
docker/scripts/compose.sh --profile '*' exec -T postgres \
...
```bash
docker/scripts/compose.sh --profile '*' exec -T pulsar \
...
docker/scripts/compose.sh --profile '*' exec -T opensearch \
...
```bash
docker/scripts/compose.sh --profile '*' exec -T hugegraph ps -ef
docker/scripts/compose.sh --profile '*' exec -T hugegraph \
