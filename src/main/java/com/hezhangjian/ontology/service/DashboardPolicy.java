...
final class DashboardPolicy {
    private static final Set<String> SOURCE_KINDS =
            Set.of("DATASET", "FUNCTION", "OBJECT_SET");
    ...
    DashboardValidationResult validate(DashboardDefinition definition) {
        ...
        for (DashboardDataSource source : sources) {
            ...
            if (!Set.of("DATASET", "OBJECT_SET").contains(source.kind()) && source.referenceId() == null) {
                issues.add(error("REFERENCE", "dataSources." + source.id(), "引用数据源必须指定当前资源"));
            }
