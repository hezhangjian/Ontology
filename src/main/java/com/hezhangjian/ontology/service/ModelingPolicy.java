...
import static com.hezhangjian.ontology.service.ModelingModels.PropertyDraft;
...
public class ModelingPolicy {
    ...
    public void validateProperties(List<PropertyDraft> properties) {
        ...
        for (PropertyDraft property : properties) {
            ...
            if (property.displayName() == null || property.displayName().isBlank()) throw problem("DISPLAY_NAME_INVALID", "属性显示名称不能为空");
            if (!displayNames.add(property.displayName().trim().toLowerCase(Locale.ROOT))) throw problem("PROPERTY_DISPLAY_NAME_CONFLICT", "属性显示名称重复");
    ...
    public void validateActionOperation(String operation) {
        if (!List.of("CLEAR_OVERRIDES", "CREATE", "DELETE", "LINK", "UNLINK", "UPDATE").contains(upper(operation))) {
