...
public class StorageHttpClient {
    ...
    private Response exchange(String method, URI uri, String body, String contentType) {
        ...
        int attempts = "POST".equals(method) ? 1 : 2;
        for (int attempt = 0; attempt < attempts; attempt++) {
            try {
                return send(method, uri, body, contentType);
            } catch (IOException exception) {
        ...
        throw unavailable(uri, lastFailure);
    }

    private Response send(String method, URI uri, String body, String contentType)
        ...
        try {
            ...
            throw new IOException("Storage response was not valid JSON", invalidJson);
        }
    }
    ...
    private ProjectionException unavailable(URI uri, Exception exception) {
        ...
        return new ProjectionException(
                "STORAGE_UNAVAILABLE",
                "Storage request failed at " + uri.getPath() + ": " + detail,
                true,
                exception);
    }
