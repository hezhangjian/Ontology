...
class DashboardPolicyTest {
    ...
    @Test
    void rejectsRemovedExplorerSourceKind() {
        DashboardDataSource source = new DashboardDataSource(sourceId, "旧探索", "EXPLORATION", objectTypeId,
                null, UUID.randomUUID(), null, Map.of());
        ...
        assertThat(policy.validate(definition).issues())
                .extracting(DashboardValidationIssue::code)
    ...
    private DashboardDefinition definition(List<DashboardWidget> widgets) {
        DashboardDataSource source = new DashboardDataSource(sourceId, "员工", "OBJECT_SET", objectTypeId, null, null, null,
                Map.of("objectTypeId", objectTypeId, "where", Map.of(), "sort", List.of(), "pageSize", 50, "columns", List.of()));
