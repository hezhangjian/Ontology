...
class PipelineGraphValidatorTest {
        ...
        PipelineGraph graph = graph(
                List.of(node("source-1", "SOURCE", Map.of()),
                        ...
                        node("output-1", "OBJECT_OUTPUT", Map.of(
                                "idFields", List.of("id"), "mappings", Map.of("displayName", "display_name"),
    ...
    void rejectsCycles() {
        PipelineGraph graph = graph(
                List.of(node("source-1", "SOURCE", Map.of()),
                        ...
                        node("output-1", "OBJECT_OUTPUT", Map.of("idFields", List.of("employee_id"), "objectTypeId", "Employee"))),
                List.of(edge("e1", "source-1", "select-1"), edge("e2", "select-1", "output-1"),
    ...
    void acceptsTwoInputJoinAndMultipleOutputs() {
        ...
        assertThat(join.outputSchema()).extracting(FieldSchema::name)
                .containsExactly("employee_id", "name", "right_employee_id", "leader");
    }

    @Test
    void joinsAggregatedRowsIntoObjectAndRelationOutputs() {
        PipelineGraph graph = graph(
                List.of(node("source-1", "SOURCE", Map.of()),
                        node("aggregate-1", "AGGREGATE", Map.of(
                                "aggregation", "SUM", "field", "employee_id",
                                "groupBy", List.of("employee_id", "name"),
                                "outputField", "token_total")),
                        node("source-2", "SOURCE", Map.of("sourceSchema", List.of(
                                Map.of("name", "employee_id", "type", "STRING"),
                                Map.of("name", "leader", "type", "STRING")))),
                        node("join-1", "JOIN", Map.of(
                                "leftKey", "employee_id", "rightKey", "employee_id",
                                "lookupPrefix", "leader_")),
                        node("object-output", "OBJECT_OUTPUT", Map.of(
                                "idFields", List.of("employee_id"), "objectTypeId", "Employee")),
                        node("link-output", "LINK_OUTPUT", Map.of(
                                "relationTypeId", "EmployeeLeader",
                                "sourceIdField", "employee_id", "targetIdField", "leader"))),
                List.of(edge("e1", "source-1", "aggregate-1"),
                        edge("e2", "aggregate-1", "join-1"),
                        edge("e3", "source-2", "join-1"),
                        edge("e4", "join-1", "object-output"),
                        edge("e5", "join-1", "link-output")));

        ValidationResult result = validator.validate(graph, PipelineMode.BATCH, sourceSchema);

        assertThat(result.valid()).isTrue();
        PipelineNode aggregate = result.normalizedGraph().nodes().stream()
                .filter(node -> node.id().equals("aggregate-1")).findFirst().orElseThrow();
        assertThat(aggregate.outputSchema())
                .filteredOn(field -> field.name().equals("employee_id"))
                .extracting(FieldSchema::sourceNodeId)
                .containsExactly("aggregate-1");
        assertThat(result.impact()).containsEntry("outputs", 2L);
    }
