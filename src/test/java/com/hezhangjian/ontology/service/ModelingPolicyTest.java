...
class ModelingPolicyTest {
    ...
    @Test
    void acceptsSupportedActionOperations() {
        for (String operation : List.of("CLEAR_OVERRIDES", "CREATE", "DELETE", "LINK", "UNLINK", "UPDATE")) {
            assertThatCode(() -> policy.validateActionOperation(operation)).doesNotThrowAnyException();
        }
    ...
    private ModelingModels.PropertyDraft property(String apiName, String type, boolean required, boolean primary, boolean title) {
        return new ModelingModels.PropertyDraft(apiName, apiName, "", type, required, primary, title,
                true, true, false, false, null, null, null, List.of(), !primary);
    }
}
